/**
 * Local sandbox check for drive.search_files via connector.execute().
 * Loads credentials from .env — never prints secrets.
 *
 * Usage:
 *   npm run test:search
 *   npm run test:search -- --q="name contains 'test'"
 *   npm run test:search -- --pageSize=5
 */
...
function parseArgs(argv: string[]): { q: string; pageSize: number; pageToken?: string } {
  let q = "trashed = false";
  let pageSize = 5;
  let pageToken: string | undefined;

  for (const arg of argv) {
    if (arg.startsWith("--q=")) q = arg.slice("--q=".length);
    else if (arg.startsWith("--pageSize=")) pageSize = Number(arg.slice("--pageSize=".length));
    else if (arg.startsWith("--pageToken=")) pageToken = arg.slice("--pageToken=".length);
  }
